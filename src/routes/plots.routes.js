...
import * as c from '../controllers/plots.controller.js';
...
router.get('/:id', loadSubscription({ write: false }), loadOwned(Plot), wrap(c.getOne));
router.patch('/:id', loadSubscription({ write: true }), loadOwned(Plot), wrap(c.update));
router.delete('/:id', loadSubscription({ write: true }), loadOwned(Plot), wrap(c.remove));
